...
class Student extends Authenticatable implements MustVerifyEmail
{
    ...
    {
        return $this->hasMany(Notification::class, 'user_id', 'id')->where('who_is', 1)->where('see', 0)->latest();
    }
    public function latestChat()
    {
        return $this->hasMany(ChatGroup::class, 'user_two', 'id')->leftJoin('chat_msg', 'group_id', '=', 'chat_group.id')->where('seen',0)->where('sender_id','author');
    }
    ...
    {
        return  $this->notify(new CustomVerifyNotification("customer"));
    }
}
