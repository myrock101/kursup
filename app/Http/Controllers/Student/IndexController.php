...
class IndexController extends Controller
{
    public function index()
    {
        ...
        $coursesfeatured = Course::with(['instructor:id,name,slug', 'category:id,name,slug', 'SubCategory:id,name,subname_ua,slug'])->where([['status', 1], ['is_featured', 1]])->inRandomOrder()->limit(10)->get();
        $courses = Course::with(['instructor:id,name,slug', 'category:id,name,slug,name_ua', 'SubCategory:id,name,subname_ua,slug'])->where('status', 1)->latest()->limit(10)->get();
    ...
    public function filter(Request $request)
    {
        ...
        $sort = $request->sort ?? 1;
        $courses = Course::with(['instructor:id,name,slug', 'category:id,name,slug', 'SubCategory:id,name'])->where('status', 1)->where('title', 'LIKE', '%' . $q . '%');
        if ($cat) {
	...
	public function ajaxfilters(Request $request)
    {
        $q = $request->q ?? "";
        ...
        if ($lan) {
            $courses->whereIn('language_id', $lan);
        }
        ...
        if ($rating || $duration) {
            ...
            if ($request->input('price_form'))
            {
                // price_form (от)
                $from = $request->input('price_form');

                $courses = $courses->filter(function ($item) use ($from) {
                    return $item->real_price >= $from;
                });
            }

            if ($request->input('price_to') && $request->input('price_to') != 50000)
            {
                // price_to (до)
                $to = $request->input('price_to');

                $courses = $courses->filter(function ($item) use ($to) {
                    return $item->real_price <= $to;
                });
            }
            ...
            $courses = Course::whereIn('id', $courses->pluck('id'));
            ...
            $courses = Course::whereIn('id', $courses->pluck('id'))->paginate(12);
        }

        return view('frontend.inc.ajaxfilter', [
            'courses' => $courses,
            'params' => $params,
            'cat' => $cat,
            'category' => $category,
    ...
    public function instructorShow($slug, Request $request)
    {
		$inst = Instructor::where('slug',$slug)->first();
		if(!$inst) abort(404);
        ...
        $ud = $request->user('customer');
        $cisd = $inst->courses->pluck('id');
        $crc = CourseRating::whereIn('course_id', $cisd)->count();
        $inst['ins_sub'] = 0;
        $inst['crc'] = $crc;
        if ($ud) {
            $s = Subscription::firstWhere([['instructor_id', $inst->id], ['student_id', $ud->id]]);
            $inst['ins_sub'] = $s ? 1 : 0;
        }
    ...
    public function coursesAll(Request $request, $type = "")
    {
        ...
        if ($type == "featured") {
            ...
            $courses = Course::with(['instructor:id,name,slug', 'category:id,name,slug', 'SubCategory:id,name'])->where([['status', 1], ['is_featured', 1]])->where('title', 'LIKE', '%' . $q . '%')->paginate(12);
        } elseif ($type == "latest") {

            $courses = Course::with(['instructor:id,name,slug', 'category:id,name,slug', 'SubCategory:id,name'])->where('status', 1)->latest()->where('title', 'LIKE', '%' . $q . '%')->paginate(12);
        } else {

            $courses = Course::with(['instructor:id,name,slug', 'category:id,name,slug', 'SubCategory:id,name'])->where('status', 1)->where('title', 'LIKE', '%' . $q . '%')->paginate(12);
        }
    ...
    public function courseShow($slug, Request $request)
    {
        ...
        $course->load(['subCategory:id,name', 'category:id,name,slug', 'content', 'content.lectures', 'instructor:id,name,slug,email,image', 'language:id,name', 'reviews.student']);
        $course->loadCount(['enroll']);
        $l = Lecture::where('course_id', $course->id)->get();
        $course['lectures_count'] = $l->count();
        $course['lectures_length'] = $this->sec2time($l->sum('duration') * 60);

        $discount = SpecialDiscount::where('course_id', $course->id)->orderBy('percentage', 'desc')->get()->filter(function ($item) {
    ...
    public function courseShare($slug)
    {
        $course = Course::where('slug',$slug)->first();
    ...
    public function categoriesCourses($slug, $category = null,Request $request)
    {
        $seo = AdminSetting::whereIn('id', [28, 29, 30, 31, 32])->get();
		$wh = [];
		...
		if($category!==null)
		{
			$subcat = SubCategory::where('slug',$category)->first();
			if(!$subcat) abort(404);
			$wh[] = ['subcategory_id', $subcat->id];
		}
		else
		{
			$wh[] = ['category_id', $cat->id];
		}
        ...
        $courses = Course::with(['instructor:id,name,slug', 'category:id,name,slug', 'SubCategory:id,name'])->where($wh)->inRandomOrder()->paginate(12);
        ...
        return view('frontend.student.category.index', [
            'courses' => $courses,
            'params' => $params,
            'cat' => $cat,
            'category' => $category,
    ...
    public function explore(Request $request)
    {
        ...
        $courses = Course::with(['instructor:id,name,slug', 'category:id,name,slug', 'SubCategory:id,name'])->where('status', 1)->inRandomOrder()->where('title', 'LIKE', '%' . $q . '%')->paginate(12);
