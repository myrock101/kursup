...
<div class="sign_form">
    ...
    @php
    $start = Request::is('login') ? 'customer/' : 'author/';
    @endphp
    @if (env("FB_LOGIN") == 1)
    ...
    @if (Request::is('login'))
    <div class="auth-flex">
        <a href="{{ url('login') }}" class="active">{{__('Пользователь')}}</a>
        <a href="{{ url('author/login') }}" class="">{{__('Автор')}}</a>
    </div>
    <form method="POST" action="{{url('login')}}">
        @else
        <div class="auth-flex">
        <a href="{{ url('login') }}" class="">{{__('Пользователь')}}</a>
        <a href="{{ url('author/login') }}" class="active">{{__('Автор')}}</a>
    </div>
        <form method="POST" action="{{url('author/login')}}">
            @endif
            @csrf
        ...
        <p class="sgntrm145">{{__('Or')}} <a
                href="{{ route('student.showResetEmailForm', ['user_type'=>'customers']) }}">{{__('Forgot
                Password')}}</a>?
        </p>
        ...
        @else
        <p class="sgntrm145">{{__('Or')}} <a
                href="{{ route('instructor.showResetEmailForm', ['user_type'=>'authors']) }}">{{__('Forgot
                Password')}}</a>?
        </p>
        <p class="mb-0 mt-30 hvsng145">{{__('Don`t have an account?')}} <a class="link-tp" href="{{ url('author/register') }}">{{__('Sign Up')}}</a></p>
        @endif
</div>
