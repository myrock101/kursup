                ...
                <div class="user_dt_right">
                    <ul>
                        ...
                        <li>
                            ...
                            <a href="{{ route('stream.likedislike', ['id'=>$live->id,'action'=>'likes']) }}" class="lkcm152 {{in_array(auth('customer')->user()->id ?? "0",$live->likes ?? []) ? 'text-danger' : ''}}
                                "><i
                                    class='uil uil-thumbs-up'></i><span>{{$live->shortNumber(count($live->likes ?? []))}}</span></a>
                        </li>
                        <li>
                            <a href="{{ route('stream.likedislike', ['id'=>$live->id,'action'=>'dislikes']) }}" class="lkcm152 {{in_array(auth('customer')->user()->id ?? "0",$live->dislikes ?? []) ? 'text-danger' : ''}}
                                "><i
            ...
            <div class="fcrse_3">
                ...
                <div class="live_comment">
                    ...
                    <input name="student_id" id="student_id" type="hidden"
                        value="{{auth('customer')->user()->id ?? 0}}" />
                    <input name="stream_id" id="stream_id" type="hidden" value="{{$live->id}}" />
                    <button class="btn_live" onclick="addComment()" {{auth('customer')->user() ? "" : "disabled"}}><i
