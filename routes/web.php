...
Route::post('messages/ajaxcheck', [ChatGroupController::class, 'ajaxcheck'])->name('message.check');
...
Route::get('blog', [\App\Http\Controllers\BlogController::class, 'index'])->name('blog.list');
Route::get('blog/{blog}', [\App\Http\Controllers\BlogController::class, 'showArticle'])->name('blog.view');
...
Route::group(['prefix' => 'admin'], function () {
    ...
    Route::group(['middleware' => ['auth', 'web']], function () {
        ...
        Route::group(['prefix' => 'report'], function () {
            Route::any('user-registration', [ReportController::class, 'studentReg'])->name('report.stureg');
            Route::any('author-registration', [ReportController::class, 'instructorReg'])->name('report.instructorreg');
...
Route::group(['prefix' => 'author'], function () {
    ...
    })->middleware('auth:author')->name('insverification.notice');
    ...
    })->middleware(['auth:author', 'web'])->name('insverification.resend');
    ...
    Route::group(['middleware' => ['auth:author', 'verified:insverification.notice']], function () {
        Route::resources([
        ...
        Route::any('help/{id}', [FAQController::class, 'faqHelpbyId']);
...
Route::any('authors', [IndexController::class, 'instructorAll'])->name('instructorAll');
Route::get('authors/{slug}', [IndexController::class, 'instructorShow'])->name('instructorShow');
...
Route::any('courses', [IndexController::class, 'coursesAll'])->name('coursesAll');
Route::any('categories/{slug}/{category}', [IndexController::class, 'categoriesCourses'])->name('subcategoriesCourses');
Route::any('categories/{slug}', [IndexController::class, 'categoriesCourses'])->name('categoriesCourses');
Route::get('courses/{slug}', [IndexController::class, 'courseShow'])->name('courseShow');
Route::get('courses/{slug}/share', [IndexController::class, 'courseShare'])->name('courseShare');
...
Route::get('author-comment/{id}/{action}', [InstructorDiscussionController::class, 'storeLikeDislike'])->name('ins.dis.social');
...
Route::delete('author-comment/{id}', [InstructorDiscussionController::class, 'destroy'])->name('ins.dis.destroy');
...
Route::post('savedevicetoken', function (Request $request) {
    if (Auth::guard('customer')->check()) {
        Student::findOrFail(Auth::guard('customer')->id())->update(['device_token' => $request->device_token]);
        return "student-done";
    } elseif (Auth::guard('author')->check()) {
        Instructor::findOrFail(Auth::guard('author')->id())->update(['device_token' => $request->device_token]);
        return "instructor-done";
    }
    return "nologin";
});
Route::group(['middleware' => ['auth:customer', 'verified:stuverification.notice']], function () {
    ...
    Route::post('author-comment', [InstructorDiscussionController::class, 'store']);
...
Route::get('/email/verify', function () {
    ...
    return view('auth.verify');
})->middleware('auth:customer')->name('stuverification.notice');
...
Route::post('/email/verify/resend', function () {
    ...
    return back()->withResent(__('A fresh verification link has been sent to your email address..'));
})->middleware(['auth:customer', 'web'])->name('stuverification.resend');
Route::get('email/verify/{id}/{hash}/{user_type}', [VerificationController::class, 'verify'])->name('ownverification.verify');
Route::post('/categories', [CategoryController::class, 'getByNameCategory']);
Route::post('/ajax/filters', [IndexController::class, 'ajaxfilters']);
Route::get('/ajax/copy/{id}', [CourseController::class, 'copyajax']);
Route::get('/categories', function (Request $request) {
    $q = $request->q ?? "";
	$params = ['q' => $q];
	return view('categories',compact('params'));
});